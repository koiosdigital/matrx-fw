 ...
 */
/*
 * KOIOS VENDORED COPY (see components/esp-tls/README.md).
 * Copied verbatim from ESP-IDF v6.0.0 with one fix in esp_mbedtls_cleanup():
 * DS-peripheral PSA keys are MBEDTLS_PK_OPAQUE, so upstream's RSASSA_PSS-gated
 * psa_destroy_key() never runs and one PSA key leaks per mTLS connection
 * attempt. Still broken in v6.0.2 — re-check before deleting this copy.
 */
#include "esp_idf_version.h"
#if ESP_IDF_VERSION != ESP_IDF_VERSION_VAL(6, 0, 0)
#error "Vendored esp-tls was copied from ESP-IDF v6.0.0. Re-sync this component with the new IDF (and re-check whether the DS PSA key leak fix below is still needed) or delete it if fixed upstream."
#endif
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#ifdef CONFIG_ESP_TLS_USE_DS_PERIPHERAL
    /* KOIOS FIX: mbedtls_pk_wrap_psa() installs mbedtls_rsa_opaque_info, whose
     * type is MBEDTLS_PK_OPAQUE — the RSASSA_PSS branch below never matches, so
     * the PSA key imported per connection in esp_mbedtls_init_pk_ctx_for_ds()
     * leaked on every attempt (mbedtls_pk_free() does not destroy wrapped PSA
     * keys). Destroy it here; pk_ctx is unused on this path. */
