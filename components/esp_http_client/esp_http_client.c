 ...
 */
/*
 * KOIOS VENDORED COPY (see components/esp_http_client/README.md).
 * Copied verbatim from ESP-IDF v6.0.0 with the v6.0.2 fix to
 * esp_http_client_connect() backported: v6.0.0 set HTTP_STATE_CONNECTING
 * before its own already-connected check, so keep-alive reuse never worked
 * and each request overwrote the previous esp_tls handle without closing it
 * (fd + heap leak). Delete this copy when moving to IDF >= 6.0.2.
 */
#include "esp_idf_version.h"
#if ESP_IDF_VERSION != ESP_IDF_VERSION_VAL(6, 0, 0)
#error "Vendored esp_http_client was copied from ESP-IDF v6.0.0 (with the 6.0.2 connect() fix backported). On IDF >= 6.0.2 delete this component; otherwise re-sync it with the new IDF."
#endif
...
static esp_err_t esp_http_client_connect(esp_http_client_handle_t client)
{
    ...
    /* KOIOS FIX (backport of the v6.0.2 change): v6.0.0 set
     * client->state = HTTP_STATE_CONNECTING here, before the
     * already-connected check below, which downgraded a kept-alive
     * HTTP_STATE_CONNECTED client on every request: keep-alive reuse never
     * happened and esp_transport_connect() overwrote the live esp_tls handle
     * without closing it, leaking the socket fd and its TLS buffers. The
     * state change now happens only in the async connect branch, as in
     * v6.0.2. */
    if (client->state < HTTP_STATE_CONNECTED) {
        ...
        if (!client->is_async) {
            ...
            client->state = HTTP_STATE_CONNECTING;  /* KOIOS FIX: moved here from above, as in v6.0.2 */
